...
{
  wrapped.set_left_tangent(PolarCoordinates(Vec2f(value.cast<std::vector<double>>())));
}

void PointWrapper::set_right_tangent(const py::object& value)
{
  wrapped.set_right_tangent(PolarCoordinates(Vec2f(value.cast<std::vector<double>>())));
}

void PointWrapper::set_position(const py::object& value)
{
  wrapped.set_position(Vec2f(value.cast<std::vector<double>>()));
}
