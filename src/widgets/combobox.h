 ...
 * Only the required methods are implemented, but the underlying QComboBox can be accessed read only.
 */
...
public:
  ...
  const QComboBox* view() const { return m_combo_box; }
...
private:
  static constexpr std::initializer_list<int> item_roles {
